...
#define OLED_SCL(x)	((x == 0)? (GPIO_PIN_RESET(OLED_SCL_PIN))	:(GPIO_PIN_SET(OLED_SCL_PIN)))
#define OLED_SDA(x)	((x == 0)? (GPIO_PIN_RESET(OLED_SDA_PIN))	:(GPIO_PIN_SET(OLED_SDA_PIN)))
#define OLED_RST(x)	((x == 0)? (GPIO_PIN_RESET(OLED_RST_PIN))	:(GPIO_PIN_SET(OLED_RST_PIN)))
#define OLED_DC(x)	((x == 0)? (GPIO_PIN_RESET(OLED_DC_PIN))	:(GPIO_PIN_SET(OLED_DC_PIN)))
#define OLED_CS(x)	((x == 0)? (GPIO_PIN_RESET(OLED_CS_PIN))	:(GPIO_PIN_SET(OLED_CS_PIN)))
...
void oled_uint16(uint8 x, uint8 y, uint16 num)
{
	...
	oled_p8x16str(x, y, &ch[1]);												// 显示数字  6*8字体
}
...
void oled_dis_bmp(uint16 high, uint16 width, uint8 *p,uint8 value)
{
	...
	for(i=0; i<temp; i++)
	{
		...
		{
			dat = 0;
			if( i<(temp-1) || !temp1 || temp1>=1)dat |= (*(p+i*8*width+j+width*0))<<0;
			if( i<(temp-1) || !temp1 || temp1>=2)dat |= (*(p+i*8*width+j+width*1))<<1;
			if( i<(temp-1) || !temp1 || temp1>=3)dat |= (*(p+i*8*width+j+width*2))<<2;
			if( i<(temp-1) || !temp1 || temp1>=4)dat |= (*(p+i*8*width+j+width*3))<<3;
			if( i<(temp-1) || !temp1 || temp1>=5)dat |= (*(p+i*8*width+j+width*4))<<4;
			if( i<(temp-1) || !temp1 || temp1>=6)dat |= (*(p+i*8*width+j+width*5))<<5;
			if( i<(temp-1) || !temp1 || temp1>=7)dat |= (*(p+i*8*width+j+width*6))<<6;
			if( i<(temp-1) || !temp1 || temp1>=8)dat |= (*(p+i*8*width+j+width*7))<<7;
...
// Sample usage:
//-------------------------------------------------------------------------------------------------------------------
//void img_dis(uint8 *p, uint8 height, uint8 width)
//{
//	uint8 i,j;
//  uint8 temp;
//	uint8 dat;
...
void oled_float(uint16 x,uint16 y,double dat,uint8 num,uint8 pointnum)
{
	uint8	length;
	int8	buff[34];
	int8	start,end,point;
...
// Sample usage:				务必注意，本函数字符列高位16，因此y应该是每次递增2
//-------------------------------------------------------------------------------------------------------------------
void oled_str(uint8 x,uint8 y,const int8 ch[])
{
	uint8 c=0,i=0,j=0;
