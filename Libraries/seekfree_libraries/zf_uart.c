...
//	uart_index[uartn]->BRR = (SystemCoreClock / baud) / 16;										// 获取波特率
//	uart_index[uartn]->FRA = (SystemCoreClock / baud) % 16;										
	
	u32 apbclock = 0x00;
	apbclock = RCC_GetPCLK2Freq();
	
	uart_index[uartn]->FRA = (apbclock / baud) % 16;										
	uart_index[uartn]->BRR = (apbclock / baud) / 16;
