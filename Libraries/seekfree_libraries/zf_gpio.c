...
//-------------------------------------------------------------------------------------------------------------------
// @brief		GPIO初始化
// @param		pin			选择的引脚 (可选择范围由 common.h 内P IN_enum 枚举值确定)
// @param		mode		引脚的方向 [GPI/GPIO]
// @param		dat			引脚初始化时设置的电平状态，输出时有效 0：低电平 1：高电平
// @param		mode		引脚的模式 (可选择范围由 zf_gpio.h 内 GPIOMODE_enum 枚举值确定)
// @return		void
...
// @param		mode		引脚的方向 [GPI/GPIO]
// @param		af			引脚的功能选择 (可选择范围由 zf_gpio.h 内 GPIOAF_enum 枚举值确定)
// @param		mode		引脚的模式 (可选择范围由 zf_gpio.h 内 GPIOMODE_enum 枚举值确定)
// @return		void
